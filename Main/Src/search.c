...
void SetWall(uint8_t x,uint8_t y){
    ...
    switch(dire_mypos){
        case north:
            w.north = _WALL_OR_NOWALL(sensor.l > detect_wall.l && sensor.r > detect_wall.r);
            w.east  = _WALL_OR_NOWALL(sensor.rr > detect_wall.rr);
            w.west  = _WALL_OR_NOWALL(sensor.ll > detect_wall.ll);
            w.south = NOWALL;
            break;
        case south:
            w.south = _WALL_OR_NOWALL(sensor.l > detect_wall.l && sensor.r > detect_wall.r);
            w.west  = _WALL_OR_NOWALL(sensor.rr > detect_wall.rr);
            w.east  = _WALL_OR_NOWALL(sensor.ll > detect_wall.ll);
            w.north = NOWALL;
            break;
        case east:
            w.east  = _WALL_OR_NOWALL(sensor.l > detect_wall.l && sensor.r > detect_wall.r);
            w.south = _WALL_OR_NOWALL(sensor.rr > detect_wall.rr);
            w.north = _WALL_OR_NOWALL(sensor.ll > detect_wall.ll);
            w.west  = NOWALL;
            break;
        case west:
            w.west  = _WALL_OR_NOWALL(sensor.l > detect_wall.l && sensor.r > detect_wall.r);
            w.north = _WALL_OR_NOWALL(sensor.rr > detect_wall.rr);
            w.south = _WALL_OR_NOWALL(sensor.ll > detect_wall.ll);
            w.east  = NOWALL;
            break;
    }
    wall[x][y] = w;
